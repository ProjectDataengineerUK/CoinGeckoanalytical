...
def _load_app_secrets() -> None:
    ...
    try:
        ...
        print(f"[cga-analytics] secrets bootstrap: token error — {exc}", file=sys.stderr, flush=True)
        return
    ...
    for env_var, (scope, key) in needed.items():
        try:
            ...
            print(f"[cga-analytics] secrets bootstrap: {scope}/{key} error — {exc}", file=sys.stderr, flush=True)
