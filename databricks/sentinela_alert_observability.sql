...
CREATE TABLE IF NOT EXISTS cgadev.ops_observability.ops_sentinela_alerts (
  kind STRING,
  message STRING,
  request_id STRING,
  tenant_id STRING,
...
CREATE OR REPLACE VIEW cgadev.ops_observability.ops_sentinela_alerts_normalized AS
SELECT
  kind,
  message,
  request_id,
  ...
  CASE
    ...
    ELSE 'other'
  END AS alert_family
FROM cgadev.ops_observability.ops_sentinela_alerts;

CREATE OR REPLACE VIEW cgadev.ops_observability.ops_sentinela_alert_backlog AS
SELECT
  kind,
  alert_family,
  COUNT(*) AS alert_count,
  MAX(created_at) AS latest_created_at
FROM cgadev.ops_observability.ops_sentinela_alerts_normalized
GROUP BY kind, alert_family;

CREATE OR REPLACE VIEW cgadev.ops_observability.ops_sentinela_alert_readiness AS
SELECT
  CASE
