...
def mini_card(x, y, w, h, title, desc, icon_kind, fill, edge, title_size=6.8,
    ...
    draw_icon(icon_kind, x + 0.35, y + h - 1.95, 1.5, edge)
    add_text(x + 2.0, y + h - 0.5, title, size=title_size, color=C_TEXT,
             weight="bold", va="top", z=6)
    if desc:
        add_text(x + 2.0, y + h - 1.1, desc, size=desc_size, color=C_MUTED,
                 va="top", z=6)
    if chip:
        round_box(x + 2.0, y + 0.25, w - 2.4, 0.55, C_WHITE, edge, lw=0.6, radius=0.18, z=4)
        add_text(x + 2.15, y + 0.52, chip, size=4.6, color=edge, weight="bold",
                 va="center", z=6)
...
# Layout
# ---------------------------------------------------------------------------
ax.text(50, 98.2, "CoinGeckoAnalytical - Arquitetura Databricks 2 Apps",
        ha="center", va="center", fontsize=21, fontweight="bold",
        color=C_TEXT, zorder=10)
ax.text(50, 96.7, "Desenho novo, mais detalhado, com recursos Databricks, observabilidade e Sentinela",
        ha="center", va="center", fontsize=10.5, color=C_MUTED, zorder=10)
...
plt.savefig("docs/assets/coingeckoanalytical-architecture-v3.png", dpi=DPI, bbox_inches="tight")
