...
ax.text(50, 96.85, "Dados externos -> Bronze/Silver/Gold -> governanca -> Genie/SQL -> cga-analytics + cga-admin -> Sentinela",
...
srcs = [
    ("globe", "CoinGecko API", "precos, OHLCV, ranks\nfrequencia: near-realtime", "#E8F1FF", "#2563EB"),
    ("globe", "DefiLlama API", "TVL e dados por chain\nnormalizacao de protocolos", "#F3E8FF", "#8B5CF6"),
    ("doc", "Docs / fontes", "conhecimento e runbooks\ninputs de contexto e curation", "#ECFDF5", "#16A34A"),
    ("cloud", "Webhooks", "eventos e gatilhos\nreprocessamento por evento", "#FFF7ED", "#EA580C"),
    ("bank", "APIs de bolsa", "camada opcional\nreferencia externa e fallback", "#FEF2F2", "#DC2626"),
]
...
add_text(ing_x + ing_w / 2, 90.2, "orquestracao, dependencias, retries, SLA e logs", size=5.0, color=C_MUTED, ha="center")
for i, (title, desc, icon, fill) in enumerate([
    ("Agendamento", "cron e dependencias", "calendar", "#FFFFFF"),
    ("Triggers", "eventos / re-runs\nrecovery e backfill", "pipeline", "#FFFFFF"),
    ("Qualidade", "DQ e retries\nthresholds e quarentena", "alert", "#FFFFFF"),
    ("Logs", "auditoria e traces\nlatencia, erro e status", "audit", "#FFFFFF"),
]):
...
add_text(ing_x + ing_w / 2, 78.2, "ingestao incremental, batch, streaming e serverless", size=5.0, color=C_MUTED, ha="center")
pipe_cards = [
    ("Declarative Pipelines", "bronze -> silver", "pipeline", "#FFFFFF"),
    ("Auto Loader", "arquivos e streaming\nschema drift e checkpoint", "cloud", "#FFFFFF"),
    ("Serverless Compute", "jobs / notebooks\nauto-scale e isolation", "bolt", "#FFFFFF"),
    ("Batch + Streaming", "mixed execution\njanela e late arrivals", "calendar", "#FFFFFF"),
]
...
add_text(ing_x + ing_w / 2, 67.2, "raw landing | limpeza | enriquecimento | marts analiticos", size=5.0, color=C_MUTED, ha="center")
med_cards = [
    ("Bronze", "raw landing / snapshots\njson, parquet e delta", "db", "#FFF3E0"),
    ("Silver", "normalize / enrich / validate\njoins, dedupe e rules", "db", "#FFFDF2"),
    ("Gold", "analytical marts / KPI views\nrankings, movers, regime", "db", "#FFF9E6"),
]
...
add_text(ing_x + ing_w / 2, 56.3, "schema evolution, DQ, freshness, lineage e governance checks", size=5.0, color=C_MUTED, ha="center")
ctrl_cards = [
    ("Schema", "versionamento\ncompatibilidade", "doc", "#FFFFFF"),
    ("DQ Rules", "validacao\nregras e excecoes", "alert", "#FFFFFF"),
    ("Freshness", "watermarks\nlags e SLA", "eye", "#FFFFFF"),
    ("Lineage", "passo a passo\nimpacto e ownership", "chain", "#FFFFFF"),
]
...
add_text(ing_x + ing_w / 2, 43.8, "streaming para eventos, micro-batch para fontes externas", size=5.0, color=C_MUTED, ha="center")
add_text(ing_x + ing_w / 2, 43.8, "streaming para eventos, micro-batch para fontes externas", size=5.0, color=C_MUTED, ha="center")
...
add_text(gx + gw / 2, 90.5, "catálogo central, policies, lineage, ownership e auditoria", size=5.0, color=C_MUTED, ha="center")
uc_cards = [
    ("Catalog", "schemas / tables / views\ndominios e ativos", "book"),
    ("Lineage", "audit / trace / ownership\nend-to-end impact", "chain"),
    ("Permissions", "row / column / tenant\nRBAC e isolamento", "shield"),
    ("Tags", "classificacao e contrato\nPII, criticidade e SLA", "doc"),
]
...
add_text(gx + gw / 2, 75.6, "experimentos, registry, champion/challenger e serving", size=5.0, color=C_MUTED, ha="center")
ml_cards = [
    ("MLflow", "experimentos / tracking\nmetrics e artefatos", "gear"),
    ("Registry", "champion / versioning\nstage e approval", "db"),
    ("Model Serving", "REST endpoint\nlatency e scaling", "dashboard"),
    ("Vector Search", "retrieval / reranking\nembeddings e filtros", "magnifier"),
]
...
add_text(gx + gw / 2, 61.4, "NLQ estruturado, query planner, caches e resposta confiavel", size=5.0, color=C_MUTED, ha="center")
serv_cards = [
    ("Genie", "NLQ em linguagem natural\nresposta governada", "speech", C_SOFT_TEAL, C_CYAN),
    ("SQL Warehouses", "queries e dashboards\ncache e custo controlado", "sql", C_SOFT_TEAL, C_CYAN),
    ("Dashboards", "KPIs e leitura rapida\npublicacao e filtros", "dashboard", C_SOFT_TEAL, C_CYAN),
    ("Observability", "freshness / traces / SLO\nlatencia e confiabilidade", "eye", C_SOFT_TEAL, C_CYAN),
]
...
add_text(ax0 + aw / 2, 90.6, "dashboards, Genie chat, copilot, confianca e provenance", size=4.8, color=C_MUTED, ha="center")
for i, (title, desc, icon) in enumerate([
    ("Genie Chat", "NLQ and guided answers\nprompt, query e confidence", "speech"),
    ("Charts", "rankings, movers, macro\nmarts, filtros e drilldowns", "dashboard"),
    ("Copilot", "routing and synthesis\norquestracao e handoff", "bot"),
    ("Freshness", "provenance badges\nsource, lag e health", "eye"),
]):
    w = (aw - 0.8) / 2
    x = ax0 + 0.2 + (i % 2) * w
...
add_text(ax0 + aw / 2, 60.0, "acesso, budget, alerts, runbooks e inspeção", size=4.8, color=C_MUTED, ha="center")
for i, (title, desc, icon) in enumerate([
    ("Ops", "status / runbooks\njobs, incidentes e SLA", "gear"),
    ("Access", "groups and isolation\nRBAC e tenants", "shield"),
    ("Audit", "SQL, traces, lineage\nprovenance e compliance", "audit"),
    ("Cost", "budgets and tokens\nllm spend e guardrails", "alert"),
]):
    w = (aw - 0.8) / 2
    x = ax0 + 0.2 + (i % 2) * w
...
sent_cards = [
    ("Coordinator", "orquestracao e merge\nfan-out e decisão", "bot", "#F8FAFC"),
    ("Watcher", "monitoring and drift\nsinais de anomalia", "eye", "#F8FAFC"),
    ("Evaluator", "quality and scoring\nrubricas e benchmarks", "shield", "#F8FAFC"),
    ("Summarizer", "reports and briefs\ninsights executivos", "doc", "#F8FAFC"),
    ("Policy", "guardrails and approvals\naceite e bloqueios", "alert", "#F8FAFC"),
    ("Alerts", "notificacoes e tickets\npaginação e incidentes", "alert", "#F8FAFC"),
    ("Logs", "audit / history / traces\ncorrelacao e replay", "audit", "#F8FAFC"),
    ("SLOs", "latency / freshness / cost\nsinal e budget", "dashboard", "#F8FAFC"),
]
...
add_text(88.2, 4.2, "SAIDA: alertas, tickets, runbooks, relatórios e ações", size=4.7,
         color=C_SENTINELA, weight="bold", ha="center", va="center")
...
plt.savefig("docs/assets/coingeckoanalytical-architecture-v7.png", dpi=DPI, bbox_inches="tight")
